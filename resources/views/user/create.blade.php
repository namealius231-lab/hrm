        ...
        <div class="form-group">
            ...
            <div class="form-icon-user">
                @php
                    $defaultRole = null;
                    if (\Auth::user()->type == 'super admin') {
                        // Default to company role for super admin
                        $companyRole = \Spatie\Permission\Models\Role::where('name', 'company')->first();
                        $defaultRole = $companyRole ? $companyRole->id : null;
                    }
                @endphp
                {!! Form::select('role', $roles, $defaultRole, ['class' => 'form-control', 'required' => \Auth::user()->type != 'super admin']) !!}
            </div>
            @error('role')
