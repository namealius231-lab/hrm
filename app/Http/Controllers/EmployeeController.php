...
class EmployeeController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('Create Employee')) {
            ...
            // Handle super admin - show records with created_by = 0 or their own ID
            if ($user->type == 'super admin') {
                ...
                $documents = Document::where('created_by', $user->creatorId())->get();
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Employee')) {
            ...
            $user = \Auth::user();
            if ($user->type == 'super admin') {
                $requiredDocuments = Document::where(function($query) use ($user) {
                    $query->where('created_by', '=', 0)
                          ->orWhere('created_by', '=', $user->creatorId());
                })->where('is_required', 1)->pluck('id')->toArray();
            } else {
                $requiredDocuments = Document::where('created_by', $user->creatorId())->where('is_required', 1)->pluck('id')->toArray();
            }
            ...
            // Only require documents that are marked as required
            foreach ($requiredDocuments as $docId) {
                $rules['document.' . $docId] = 'required';
            }
            ...
            // Assign employee role - create if it doesn't exist
            $employeeRole = \Spatie\Permission\Models\Role::firstOrCreate(
                ['name' => 'employee'],
                ['created_by' => \Auth::user()->creatorId()]
            );
    ...
    public function getdepartment(Request $request)
    {
        ...
        if ($request->branch_id == 0) {
            if ($user->type == 'super admin') {
                ...
                $departments = Department::where('created_by', '=', $user->creatorId())->get()->pluck('name', 'id')->toArray();
            }
        } else {
            if ($user->type == 'super admin') {
                $departments = Department::where(function($query) use ($user) {
                    $query->where('created_by', '=', 0)
                          ->orWhere('created_by', '=', $user->creatorId());
                })->where('branch_id', $request->branch_id)->get()->pluck('name', 'id')->toArray();
            } else {
                $departments = Department::where('created_by', '=', $user->creatorId())->where('branch_id', $request->branch_id)->get()->pluck('name', 'id')->toArray();
            }
        }
    ...
    public function json(Request $request)
    {
        ...
        if ($request->department_id == 0) {
            if ($user->type == 'super admin') {
                ...
                $designations = Designation::where('created_by', '=', $user->creatorId())->get()->pluck('name', 'id')->toArray();
            }
        } else {
            if ($user->type == 'super admin') {
                $designations = Designation::where(function($query) use ($user) {
                    $query->where('created_by', '=', 0)
                          ->orWhere('created_by', '=', $user->creatorId());
                })->where('department_id', $request->department_id)->get()->pluck('name', 'id')->toArray();
            } else {
                $designations = Designation::where('department_id', $request->department_id)->where('created_by', '=', $user->creatorId())->get()->pluck('name', 'id')->toArray();
            }
        }
