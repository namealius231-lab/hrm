    ...
    public function index()
    {
        if (\Auth::user()->can('Manage Competencies')) {
            ...
            if ($user->type == 'super admin') {
                $competencies = Competencies::where(function($query) use ($user) {
                    $query->where('created_by', '=', 0)
                          ->orWhere('created_by', '=', $user->creatorId());
                })->with('getPerformance_type')->get();
            } else {
                $competencies = Competencies::where('created_by', $user->creatorId())->with('getPerformance_type')->get();
            }
